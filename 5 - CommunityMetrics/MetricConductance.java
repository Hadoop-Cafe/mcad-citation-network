...
public class MetricConductance {
	...
	public static class PaperEntryReducer extends TableReducer<Text, Text, ImmutableBytesWritable> {
		...
		protected void setup(Reducer<Text, Text, ImmutableBytesWritable, Mutation>.Context context)
			...
			HTable htableCommunityAbout = new HTable(config, "communityNetwork");
			
			Scan scan = new Scan();
			ResultScanner scanner = htableCommunityAbout.getScanner(scan);
			
			for (Result result = scanner.next(); (result != null); result = scanner.next()) {
				String fromClass = Bytes.toString(result.getValue(Bytes.toBytes("from"), Bytes.toBytes("fromClass")));
				String toClass = Bytes.toString(result.getValue(Bytes.toBytes("to"), Bytes.toBytes("toClass")));
				if (fromClass.equals(toClass)) {
		...
		protected void reduce(Text fromClass, Iterable<Text> counts, Context context)
			...
			put.add(Bytes.toBytes("metrics"), Bytes.toBytes("conductance"), Bytes.toBytes(Double.toString(conductance)));
