def write_csv(metric, start_year, end_year_plus_one, interval, metric_dict):
	...
	for year in range(start_year, end_year_plus_one, interval):
		...
		for comm in all_comm:
			if (str(year), comm, metric) in metric_dict.keys():
				csv.write(metric_dict[(str(year), comm, metric)] + ',')
			else:
				csv.write('0,')
		csv.write('\n')
...
print lines[0]
lines = [line.split(' ')[0][0:4] + ' ' + line.split(' ')[0][10:] + line[line.find(' '):] for line in lines]
print lines[0]
...
for line in lines:
	...
	metric_dict[(year, comm, metric)] = value



metrics = ['inwardness', 'cut', 'expansion', 'count']
start_year = 1960
end_year_plus_one = 2016
interval = 1
